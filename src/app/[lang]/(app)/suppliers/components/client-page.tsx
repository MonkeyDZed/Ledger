  ...
  const headerContent = (
    <>
      {isMounted ? (
                ...
                <div className="flex items-baseline gap-4">
                  ...
                  {!isHeaderOpen && (
                    <div className="hidden md:flex items-center gap-4 text-sm text-muted-foreground font-mono">
                      ...
                      <span suppressHydrationWarning>Créances: <span className="font-bold text-rose-700">{formatCurrencyWithLocale(totals.totalDebt, lang)}</span></span>
                    </div>
                  )}
                </div>
        ...
        <div className="mb-4">
          <PageHeader title={dictionary.title} description={dictionary.description}>
            <Button variant="outline" onClick={() => {}}>
              <FileDown className="me-2 h-4 w-4" />
              {dictionary.export}
            </Button>
