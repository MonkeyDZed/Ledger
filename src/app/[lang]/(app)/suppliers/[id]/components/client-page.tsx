...
export function ClientPage({ supplier, pieces, dictionary, supplierFormDictionary, addPieceAction, updatePieceAction, deletePieceAction, updateSupplierAction }: ClientPageProps) {
  ...
  const facturesCount = pieces.filter(p => p.type !== 'VERSEMENT').length;
                ...
                <div className="flex items-baseline gap-4">
                  ...
                  {!isHeaderOpen && (
                    <div className="hidden md:flex items-center gap-4 text-sm text-muted-foreground font-mono">
                      <span>{facturesCount} {dictionary.stats.pieces || 'pièces'}</span>
                      <span className="h-4 border-l"></span>
                      <span suppressHydrationWarning>Dette: <span className="font-bold text-rose-700">{formatCurrencyWithLocale(totalDebt, lang)}</span></span>
                    </div>
                  )}
                </div>
